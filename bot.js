...
bot.on('guildMemberAdd', member => {
  member.createDM().then(DMChannel => {
    ...
    welcomeMsg += "Par exemple, si tu es en Info1 et dans le groupe A, tapes `!role info1 a` dans le salon #bienvenue.\n";
...
function setRole(message, args) {
  message.react(message.guild.emojis.find((emoji) => emoji.name === "party_wumpus"));
  ...
  if (args.length === 0) {
    ...
    const embed = new Discord.RichEmbed()
        ...
        .setFooter('N\'hésitez pas à demander de l\'aide si besoin ! Toute suggestion est la bienvenue.');
    ...
    if (roleAlias in roleList) {
      ...
      for (let role of roles) {
        if (message.member.roles.some(r => r.name == role)) {
          ...
          message.member.addRole(message.guild.roles.find((r) => r.name === role));
